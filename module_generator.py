...
class Module:
	...
	def generateModelHeader(self, stream):		
		...
		stream.write('\tstd::function<std::unique_ptr<AudioGraphNode> ()> getAudioFactory() override;\n')
	...
	def generateModelSource(self, stream):		
		...
		stream.write('std::function<std::unique_ptr<AudioGraphNode> ()> {0}_Model::getAudioFactory()\n{{\n'.format(self.name))
	...
	def generateAudioModuleSource(self, stream):
		...
		stream.write('\t\t//Set configuration here\n')
		...
		stream.write('\tif(')
		for input in self.inputs:
			stream.write('!{0}Buffer || '.format(input))
		for output in self.outputs:
			stream.write('!{0}Buffer || '.format(output))
		stream.write(')\n\t{\n\t\treturn;\n\t}\n\n')
	...
	def	generateNamespace(self, stream):
		...
		stream.write('const static String ModuleName = "{0}";\n\n'.format(self.name))

		for count, input in enumerate(self.inputs):
			stream.write('const static int {0} = {1};\n'.format(input, count))
		stream.write('\n')
